...
#define TIMER_CONTROL ((unsigned char*)0xC210)  /* Control register */
#define TIMER_LOW     ((unsigned char*)0xC211)  /* Counter low byte */
#define TIMER_HIGH    ((unsigned char*)0xC212)  /* Counter high byte */
 ...
 */
unsigned int time_function(void (*func)(void)) {
...
void run_timing_tests(void) {
    ...
    microseconds = ticks_to_microseconds(execution_time);
    
    printf("delay_short():\n");
    printf("  Ticks: %u\n", execution_time);
    printf("  Time: %lu microseconds\n", microseconds);
    printf("  Time: %u milliseconds\n\n", ticks_to_milliseconds(execution_time));
    ...
    printf("  Manual timing: %lu microseconds\n", ticks_to_microseconds(execution_time));
}
...
void benchmark_sd_operations(void) {
    ...
    printf("Block read: %u ticks (%lu us)\n", 
           read_time, ticks_to_microseconds(read_time));
    ...
    printf("Block write: %u ticks (%lu us)\n", 
           write_time, ticks_to_microseconds(write_time));
}
